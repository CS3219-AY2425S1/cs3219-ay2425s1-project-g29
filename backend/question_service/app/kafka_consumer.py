...
consumer_config = {
    ...
    "group.instance.id": "question-processor-instance",
}
...
def find_matching_question(target_category, difficulty):
    ...
    query = (
        questions_ref.where("category", "array_contains", target_category)
    ...
    category_query = questions_ref.where(
        "category", "array_contains", target_category
    ).limit(1)
...
def process_match_result(match_result):
    ...
    if matching_question is None:
        print(
            f"No matching question found for category: {category}, difficulty: {difficulty}"
        )
